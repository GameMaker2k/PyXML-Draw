...
parser.add_argument("-t", "--outputtype", default="png",
...
parser.add_argument(
    ...
    "--resizetype",
    default="nearest",
    help="Resize algorithm (nearest, bilinear, bicubic, antialias)")
...
def process_shape(xml_img, shape_data, shape_type):
    ...
    fill = colortolistalpha(
        shape_data['fill'], int(
            shape_data.get(
                'alpha', 255)))
    outline = colortolistalpha(
        shape_data.get(
            'outline', '#000000'), int(
            shape_data.get(
                'outlinealpha', 255)))
...
def xml_draw_image(
    ...
    # Parse XML
    tree = ET.ElementTree(file=xiffile) if os.path.isfile(
        xiffile) else ET.ElementTree(ET.fromstring(xiffile))
    root = tree.getroot()
    ...
    for child in root:
        ...
        if shape_type in ['rectangle', 'ellipse', 'polygon']:
            ...
            xml_img.text(
                (int(
                    shape_data['x']), int(
                    shape_data['y'])), shape_data['text'], font=font, fill=colortolistalpha(
                    shape_data['fill'], int(
                        shape_data['alpha'])))
        elif shape_type == 'barcode':
            barcode_image = upcean.barcodes.shortcuts.validate_draw_barcode(
                type=shape_data['type'], upc=shape_data['code']).convert('RGBA')
            pre_xml_img.paste(
                barcode_image, (int(
                    shape_data['x']), int(
                    shape_data['y'])), barcode_image)

    # Resize if required
    if resize_factor > 1:
        pre_xml_img = resize_image(
            pre_xml_img,
            (width * resize_factor,

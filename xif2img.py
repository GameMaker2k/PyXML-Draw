...
def colortolist(color):
    if (re.findall("^\\#", color)):
        ...
        return (int(colorsplit[0], 16), int(
            colorsplit[1], 16), int(colorsplit[2], 16))
...
def colortolistalpha(color, alpha):
    if (re.findall("^\\#", color)):
        colorsplit = re.findall(
            "^\\#([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{2})", color)[0]
        return (int(colorsplit[0], 16), int(
            colorsplit[1], 16), int(colorsplit[2], 16), int(alpha))
    if (re.findall("^rgb", color)):
        colorsplit = re.findall(
            "^rgb\\(([0-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5]),([0-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5]),([0-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])\\)",
            color)[0]
...
def xml_draw_image(
    ...
    if (resizetype != "antialias" and resizetype !=
        ...
        resizetype = "nearest"
    if (not os.path.isfile(xiffile) and re.findall(
            "^(http|https)\\:\\/\\/", xiffile)):
    ...
    if (os.path.isfile(xiffile) and not re.findall(
            "^(http|https)\\:\\/\\/", xiffile)):
        tree = cElementTree.ElementTree(file=xiffile)
    if (not os.path.isfile(xiffile) and not re.findall(
            "^(http|https)\\:\\/\\/", xiffile)):
    ...
    for child in root:
        ...
        if (child.tag == "bitmap"):
            ...
            for coordinates in child.iter('coordinates'):
                ...
                if (sublist is None):
                    ...
                    sublist = (int(coordinates.attrib['x']), int(
                        coordinates.attrib['y']))
            if (not os.path.isfile(child.attrib['file']) and re.findall(
                    "^(http|https)\\:\\/\\/", child.attrib['file'])):
                xmlheaders = {'User-Agent': useragent_string}
                    ...
                    StringIO(
                        urllib2.urlopen(
                            urllib2.Request(
                                child.attrib['file'],
                                None,
                                xmlheaders)).read())).convert('RGBA')
            if (os.path.isfile(child.attrib['file']) and not re.findall(
                    "^(http|https)\\:\\/\\/", child.attrib['file'])):
                tmp_img_paste = Image.open(
            ...
            if ((tmp_img_paste.size[0] < int(child.attrib['width']) or tmp_img_paste.size[0] > int(child.attrib['width'])) or (
                    tmp_img_paste.size[1] < int(child.attrib['height']) or tmp_img_paste.size[1] > int(child.attrib['height']))):
                if (resizetype == "antialias"):
                    new_img_paste = tmp_img_paste.resize(
                        (int(
                            child.attrib['width']), int(
                            child.attrib['height'])), Image.ANTIALIAS)
                if (resizetype == "bilinear"):
                    new_img_paste = tmp_img_paste.resize(
                        (int(
                            child.attrib['width']), int(
                            child.attrib['height'])), Image.BILINEAR)
                if (resizetype == "bicubic"):
                    new_img_paste = tmp_img_paste.resize(
                        (int(
                            child.attrib['width']), int(
                            child.attrib['height'])), Image.BICUBIC)
                if (resizetype == "nearest"):
                    new_img_paste = tmp_img_paste.resize(
                        (int(
                            child.attrib['width']), int(
                            child.attrib['height'])), Image.NEAREST)
                xml_img.bitmap(sublist, new_img_paste,
                               fill=child.attrib['fill'])
                del (new_img_paste)
            if (tmp_img_paste.size[0] == int(
                    child.attrib['width']) and tmp_img_paste.size[1] == int(child.attrib['height'])):
        ...
        if (child.tag == "chord"):
            ...
            for coordinates in child.iter('coordinates'):
                ...
                if (sublist is None):
                    ...
                    sublist = (int(coordinates.attrib['x']), int(
                        coordinates.attrib['y']))
            xml_img.chord(sublist,
                          int(child.attrib['start']),
                          int(child.attrib['end']),
                          fill=child.attrib['fill'],
                          outline=child.attrib['outline'])
            ...
            for coordinates in child.iter('coordinates'):
                ...
                if (sublist is None):
                    ...
                    sublist = (int(coordinates.attrib['x']), int(
                        coordinates.attrib['y']))
            child.attrib['fill'] = colortolistalpha(
                child.attrib['f/usr/share/pixmaps/acidrip.pngill'],
                child.attrib['alpha'])
        ...
        if (child.tag == "multilinetext"):
            ...
            xml_img.multiline_text(
                sublist,
                mltextstrg,
                fill=child.attrib['fill'],
                font=tmp_ttf_file,
                spacing=int(
                    child.attrib['spacing']),
                align=child.attrib['align'])
            del (tmp_ttf_file)
    ...
    if (resize > 1):
        if (resizetype == "antialias"):
            new_xml_img = pre_xml_img.resize(
                (int(
                    root.attrib['width']) * int(resize), int(
                    root.attrib['height']) * int(resize)), Image.ANTIALIAS)
        if (resizetype == "bilinear"):
            new_xml_img = pre_xml_img.resize(
                (int(
                    root.attrib['width']) * int(resize), int(
                    root.attrib['height']) * int(resize)), Image.BILINEAR)
        if (resizetype == "bicubic"):
            new_xml_img = pre_xml_img.resize(
                (int(
                    root.attrib['width']) * int(resize), int(
                    root.attrib['height']) * int(resize)), Image.BICUBIC)
        if (resizetype == "nearest"):
            new_xml_img = pre_xml_img.resize(
                (int(
                    root.attrib['width']) * int(resize), int(
                    root.attrib['height']) * int(resize)), Image.NEAREST)
        del (xml_img)
        del (pre_xml_img)
